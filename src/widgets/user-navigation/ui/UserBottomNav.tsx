...
const UserBottomNav = () => {
  ...
  return (
    <nav className="bg-neutral-0 fixed inset-x-0 bottom-0 z-10 mx-auto flex h-[95px] max-w-[480px] items-start justify-center border border-neutral-200 pt-3">
      {navigationItems.map(({ href, icon, label }) => {
        ...
        return (
          <Link
            className={`flex h-12 w-[100px] flex-col items-center rounded-full text-sm leading-5 font-semibold min-[430px]:w-[120px] ${isActive ? 'bg-primary-50 text-primary-300' : 'text-neutral-600'}`}
            href={href}
            key={href}
          >
            <Image
