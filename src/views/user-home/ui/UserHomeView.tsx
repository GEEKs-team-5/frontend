...
const medications = [
  { action: '복용했어요!', active: true },
  { action: '복용 완료', active: false },
  { action: '복용했어요!', active: true },
  { action: '복용했어요!', active: true },
];
  ...
  <main className="min-h-dvh bg-neutral-100 pb-24">
    <div className="mx-auto max-w-[393px] px-5 pt-[62px]">
      ...
      <section className="bg-neutral-0 mt-5 rounded-xl px-5 py-5 text-center">
        <p className="text-sm text-neutral-600">(오후 7:00 저녁약)</p>
        <h1 className="text-neutral-1000 text-xl font-semibold">
          다음 약 복용까지 <span className="text-primary-400 text-2xl">2시간</span> 남았어요
        </h1>
      </section>
      <section className="mt-5 space-y-0" aria-label="오늘의 복약 목록">
        {medications.map(({ action, active }, index) => (
          <article className="bg-neutral-0 relative overflow-hidden px-5 pt-9 pb-5" key={index}>
            ...
            <div className="flex gap-5">
              ...
              <div className="min-w-0 pt-1">
                <h2 className="text-neutral-1000 truncate text-xl font-semibold">약 이름</h2>
            ...
            <button
              className={`mt-[17px] h-10 w-full rounded-full text-base font-semibold ${active ? 'bg-primary-400 text-neutral-0 shadow-[0_6px_0_#1c8dd3]' : 'text-neutral-0 bg-neutral-400'}`}
              type="button"
            >
              {action}
            </button>
